...
int check_arg (char *buffer)
{
  ...
  for (i=0; i < (int) strlen (buffer); i++)
  {
    ...
    if (temp > 57 || temp < 48)
      {
        cerr << "ERROR. Invalid argument (includes non-digit character). \n"; 
        return -1;
      }
...
int sem_timed_wait(int id, short unsigned int num, time_t seconds)
{
  ...
  {
    .tv_sec = seconds, // Seconds: time_t tv_sec
    .tv_nsec = 0, // Nanoseconds: long tv_nsec
  }; 

  // As above in timedwait
  struct sembuf op[] = 
  {
    {num, -1, SEM_UNDO}
  };

  // Semtimedop will return error if time runs out
  return semtimedop(id, op, 1, &time_spec); 
}
